...
docvars(my_corpus, "index") <- char_index %>% as.integer()
...
base_out <- read_rds(here("outputs", "base_out.rds"))
...
ggsave(here("outputs", "topic_modeling_base.png"), height = 10)
...
dynamic_out <- keyATM(docs = keyATM_docs,    # text input
              ...
              model_settings = list(time_index = docvars(my_corpus)$month,
                                    num_states = 5),
              options = list(seed = 250, store_theta = TRUE, thinning = 5))
 
write_rds(dynamic_out, here("outputs", "dynamic_out.rds"))

fig_timetrend <- plot_timetrend(dynamic_out, time_index_label = docvars(my_corpus)$month, xlab = "Month")

fig_timetrend

ggsave(here("outputs", "dynamic_topic.png"))
```
