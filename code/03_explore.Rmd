...
df <- df %>%
    mutate(full_text = str_to_lower(full_text),
           ...
           anti_asian = as.numeric(str_detect(full_text, "wuhanvirus|chinesevirus|chinavirus|wuhancoronavirus|wuhanpneumonia")))
...
twitter <- bind_rows(mutate(add_normalized(df, asian), Terms = "Asian"), 
          ...
          mutate(add_normalized(df, anti_asian), Terms = "Anti-Asian")
          )
...
p4 <- stacked_area_plot(df, anti_asian) +
        labs(x = "Date",
             y = "Proportion",
             fill = "Anti-Asian",
             title = "Anti-Asian",
