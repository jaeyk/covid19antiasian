...
pacman::p_load(data.table, # for fast data manipulation
               ...
               gifski) # for saving animated data viz objects 
...
df <- df %>%
    mutate(full_text = str_to_lower(full_text),
           ...
           asian = as.numeric(str_detect(full_text, "asia|asian|aapi")),
...
gtrends <- gtrends %>% dplyr::filter(keywords == "Chinese virus" |
                   keywords == "Chinese flu" | 
                   keywords == "Kung Flu" |
                   keywords == "Wuhan virus") %>%
    mutate(keywords = dplyr::recode(keywords, 
                             "Kung Flu" = "Kung flu")) %>%
    dplyr::rename(Terms = keywords)
...
anim_save(here("outputs", "animated_twitter_plot.gif"))
...
p6 <- stacked_area_plot(df, anti_racism) +
        labs(x = "Date",
             y = "Proportion",
             fill = "Wuhan virus",
             title = "Wuhan virus",
...
p7 <- stacked_area_plot(df, anti_racism) +
        labs(x = "Date",
             y = "Proportion",
             fill = "Anti-racism",
             title = "Anti-racism",
